<template>
    ...
    >
        ...
        <f7-list
        media-list
        v-for="(month, key) in Object.keys(unpaidListData)"
        :key='key'
        class="list-content"
        >
            ...
            <f7-list-item
                ...
                v-for="(item, index) in unpaidListData[month]"
                :key='index'
                link="#"
        ...
        <div class="no-data" v-if="Object.keys(unpaidListData).length==0">
...
export default {
    data(){
        return{
            unpaidPageIndex: 0, // 未抄表请求页码
            state: 'unpaid', // 状态数据
            unpaidListData: [], // 未抄表数据列表
            unpaidCount: 0, // 未抄表计数
    ...
    methods: {
        ...
        parseDictionaryData(typeOut, type, name) {
            const self = this
            console.warn(typeOut, type, name)
        ...
        requestListDataByState(done = function() {}) {
            ...
            lingFetch(
                ...
                // success
                    ...
                    self.unpaidListData = _resData.bill_list
                    self.unpaidCount = _resData.count
                    self.unpaidAllowInfinite = true;
                    self.showPreloader = false;
                    self.unpaidPageIndex = 0
                    done();
                },
                function (result) {
                // error
        ...
        loadMore() {
            const self = this;
            console.warn('loading more ----', self.unpaidAllowInfinite)
            if (!self.unpaidAllowInfinite) return;
            self.unpaidAllowInfinite = false;
            console.warn('loading more ---   1-')

            if (self.unpaidListData.length >= self.unpaidCount) {
                self.showPreloader = false;
                return;
            }
            ...
            lingFetch(
                ...
                "bill_list",{
                    state: self.state,
                    offset: 10 * (++self.unpaidPageIndex)
                },
                function (result) {
                // success
                    const _resData = result.data
                    ...
                    var listKeys = Object.keys(self.unpaidListData),
                        listKeysTmp = Object.keys(_resData.bill_list)
                    if(listKeys[listKeys.length - 1] === listKeysTmp[0]){
                        self.unpaidListData[listKeys[listKeys.length - 1]] = self.unpaidListData[listKeys[listKeys.length - 1]].concat(_resData.bill_list[listKeysTmp[0]])
    ...
    mounted() {
        ...
        console.log('router :', self.$f7router);
    }
}
</script>
