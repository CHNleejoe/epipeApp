...
import col from "framework7-vue/components/col";
 ...
 */
function lingFetch(self, api, method, args, success, error, options) {
    if (!self.$u.isNetEnable()) {
        self.$v.showToast(self, '您已离线')
        return false
    }
    ...
    if (options && 'url' in options) {
        param.url = envConfig.HOST + options.url
        delete param.data.args
        param.data = {...param.data, ...args }
        console.log(param.data, args)
            // param.data = args
    }
