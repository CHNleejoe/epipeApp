...
var session = {
    user: 'lingling',
    domain: 'im.misterling.cn/lovequest'
};
...
var query_protocal = function (name) {
    var stanza = $iq({
            from: jid,
            to: 'lingling@im.misterling.cn/lovequest',
            id: connect.getUniqueId(),
            type: 'set'
        }).c('query', {
            xmlns: 'http://jabber.org/protocal/bytestreams',
            sid: 'dv917fb4',
            mode: 'tcp'
        })
        .c('streamhost', {
...
window.CHAT_STATUS = CHAT_STATUS;
var Chat = {
    changeStatus: function (status, connection, from, to) {
        var msg = $msg({
                from: from,
                to: to,
                type: 'chat',
